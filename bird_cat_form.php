...
<div class="topnav" style="padding-bottom: 0px;">
    ...
    <h4 style="display: inline; color: rgba(255,255,255,0.9); "> Bird Categories </h4>
</div>
<div class="custombutton">         
    <button class="clicks-btn" onclick="addNew()">
        Add new bird category
    </button>
...
<?php
    ...
    if($con){
        $var = mysqli_query($con,"select * from ".$table_name." where deleted = 0");
        ?>
        <table style="width: fit-content;">
            ...
            <tr style="color: #fff;">
                ...
                <th class="col-g">Add SubTypes</th>
            </tr>
            </thead>
        <?php
            ...
            <tr  class="col-a" style="<?php echo ($arr['deleted'] == 1 ? 'display: none;' : 'display: block;'); ?> /*width: fit-content;*/" >
                ...
                <td class="col-g">
                    <button 
                        class="clicks-btn"
                        onclick="addTypes(<?php echo ($arr['id']); ?>)">
                        Add Types
                    </button>
                </td>
            </tr>
...
<div class="entry_form" style="display: none;">
    ...
    <form method="post" style="width: 50%; margin-left: 50px;"> 
        <fieldset> 
            ...
            <input type="text" name="name" placeholder="Enter bird category name" class="input-el row-b" required>
            <br>

            <input type="text" name="description" placeholder="Enter bird category description"  class="input-el row-c" >
            <br>
...
<script>
    ...
    function addNew(){
        $('.entry_form').show()
        $('.entry_form').find('input:not([name="table_name"])').val('')
    ...
    function refreshInnerPanel(){
        $('.nav-bar').attr('class', 'nav-bar nav-sticky')

        let loadPage = 'bird_cat_form.php';
        $('.refresh-label').show()
        fetch(loadPage, {
            method: 'POST',
            body: JSON.stringify({
    ...
    function goBack() {
        let loadPage = '<?php echo $gotoMain ?>'+'entry_panel.php';
