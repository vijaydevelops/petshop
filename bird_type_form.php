...
<select name="bird_cat_selected" onchange="refreshInnerPanel(this, event)" >
    <option value="" >Select a bird category</option>
    <?php 
        $var = mysqli_query($con,"select * from bird_cat where deleted = 0");
        if(mysqli_num_rows($var)>0){
            ...
            ?>
            <option value="<?php echo $arr['id']; ?>" <?php echo ($bird_cat==$arr['id'] ? 'selected' : '');  ?> >Showing types of <?php echo $arr['name']; ?></option>
            <?php 
            }
        }
    ?>
...
<div class="custombutton">         
    <button class="clicks-btn" onclick="addNew()">
        Add new type of <?php echo $bird_cat_name; ?>
    </button>
...
<?php
    ...
    if($con){
        $var = mysqli_query($con,"select * from ".$table_name." where deleted = 0 and bird_cat_id = '". $bird_cat ."'");
        ?>
...
<div class="entry_form" style="display: none;">
    ...
    <form method="post" style="width: 50%; margin-left: 50px;"> 
        <fieldset> 
            ...
            <input type="hidden" name="bird_cat_id" class="row-1" value="<?php echo $bird_cat; ?>">
            ...
            <input type="text" name="name" placeholder="Enter <?php echo $bird_cat_name; ?> type name" class="input-el row-b" required>
            <br>

            <input type="text" name="description" placeholder="Enter <?php echo $bird_cat_name; ?> type description"  class="input-el row-c" >
            <br>
...
<script>
    ...
    function addNew(){
        $('.entry_form').show()
        $('.entry_form').find('input:not([name="table_name"]):not([name="bird_cat_id"])').val('')
    ...
    function refreshInnerPanel(el, ev){
        ...
        let loadPage = 'bird_type_form.php';
        $('.refresh-label').show()
        fetch(loadPage, {
            method: 'POST',
            body: JSON.stringify({
                gotoMain: (<?php echo json_encode($gotoMain); ?>) ,
                bird_cat : el ? el.value : $('[name="bird_cat_selected"]')[0].value
            }),
    ...
    function addPets(type_id) {
        let loadPage = '<?php echo $gotoMain ?>'+'bird_form.php';
    ...
    function goBack() {
        let loadPage = '<?php echo $gotoMain ?>'+'bird_cat_form.php';
