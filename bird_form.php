...
<?php
    ...
    $table_name = 'birds';
    $bird_type = '';
    $bird_type_name = 'bird type';

    if(isset($posted_json['bird_type'])) {
        $bird_type = $posted_json['bird_type']; 
    }

    $bird_cat = '';
    $bird_cat_name = 'bird';

    if(isset($posted_json['bird_cat'])) {
        $bird_cat = $posted_json['bird_cat']; 
    }

    if($bird_cat == ''){
        $var = mysqli_query($con,"select id, name from bird_cat where deleted = 0 and id='".$bird_cat."'");
        if(mysqli_num_rows($var)>0){
            // $i=0;
            while($arr=mysqli_fetch_assoc($var))
            {
                $bird_cat_name = $arr['name'];
            }
        }
    }
...
<div class="topnav" style="padding-bottom: 0px;">
    ...
    <h4 style="display: inline; color: rgba(255,255,255,0.9); "> BIRDS </h4>
</div>

<?php 
    $qry =  "select bt.id, bt.bird_cat_id, bt.name, bc.name as ani_cat_name
                    from bird_type as bt
                    left join bird_cat  as bc
                    on bt.bird_cat_id = bc.id
                    where bt.deleted = 0 and bt.id='".$bird_type."'";
    // var_dump($qry);

 ?>

<select name="bird_type_selected" onchange="refreshInnerPanel(this, event)" >
    <option value="" >Select a <?php echo strtolower($bird_cat_name); ?> type</option>
    <?php 
    ...
    if(mysqli_num_rows($var)>0){
        ...
        {
            if($bird_type==$arr['id'])
                $bird_type_name = $arr['name'];
            if($bird_cat == '')
                $bird_cat = $arr['bird_cat_id'];
            if($bird_cat_name == '' || $bird_cat_name == 'bird')
                $bird_cat_name = $arr['ani_cat_name'];
        ?>
        <option value="<?php echo $arr['id']; ?>" <?php echo ($bird_type==$arr['id'] ? 'selected' : '');  ?> >Showing all of <?php echo $arr['name']; ?></option>
        <?php 
        }
    }
    ?>
...
<div class="custombutton">         
    <button class="clicks-btn" onclick="addNew()">
        Add new pet under <?php echo $bird_type_name; ?>
    </button>
...
<?php
    ...
    if($con){
        $var = mysqli_query($con,"select * from ".$table_name." where deleted = 0 and bird_type_id = '". $bird_type ."'");
        ?>
        <table style="width: fit-content;">
            <thead>
                <tr style="color: #fff;">
                    ...
                    <th class="col-i">Bird Pic Link</th>
        ...
        <?php
            ...
            <tr  class="col-a" style="<?php echo ($arr['deleted'] == 1 ? 'display: none;' : 'display: block;'); ?> /*width: fit-content;*/" >
                ...
                <td class="col-i" data="<?php echo $arr['bird_pic']; ?>"> <a href="<?php echo $gotoMain. $arr['bird_pic']; ?>" target="_blank"><?php echo substr($arr['bird_pic'], strrpos($arr['bird_pic'], '/') + 1 ); ?></a> </td>
...
<div class="entry_form" style="display: none;">
    ...
    <form method="post" style="width: 50%; margin-left: 50px;"> 
        <fieldset> 
            ...
            <input type="hidden" name="bird_type_id" class="row-1" value="<?php echo $bird_type; ?>">

            <?php // var_dump($bird_cat); ?>

            <p class="show-id-box" style="display: none;" > ENTRY ID : <span class="show-id"></span></p>
            <p class="show-id-box" style="display: none;" > BIRD CATEGORY : <span class="show-anicatname"><?php echo $bird_cat_name; ?></span></p>
            <p class="show-id-box" style="display: none;" > BIRD TYPE : <span class="show-anitypename"><?php echo $bird_type_name; ?></span></p>
            ...
            <input type="file" name="bird_pic" class="input-el row-d" >
...
<script>
    ...
    function addNew(){
        $('.entry_form').show()
        $('.entry_form').find('input:not([name="table_name"]):not([name="bird_cat_id"]):not([name="bird_type_id"])').val('')
    ...
    function refreshInnerPanel(el, ev){
        ...
        let loadPage = 'bird_form.php';
        $('.refresh-label').show()
        fetch(loadPage, {
            method: 'POST',
            body: JSON.stringify({
                gotoMain: (<?php echo json_encode($gotoMain); ?>) ,
                bird_cat : (<?php echo json_encode($bird_cat); ?>) ,
                bird_type : el ? el.value : $('[name="bird_type_selected"]')[0].value
            }),
    ...
    function goBack() {
        let loadPage = '<?php echo $gotoMain ?>'+'bird_type_form.php';
